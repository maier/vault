...
RoleID is a credential to be used at the login endpoint.
The credentials used to fetch a Vault token depends on the configured
contraints on the App role. But, the the credential `role_id` is a
required argument for the login endpoint at all times. The RoleIDs
...
(other) credentials supplied. RoleID for an App role can be fetched
via `role/<role_name>/role-id` endpoint.
...
with `role_id`. The backend supports both creation of SecretID by
...
`role_id` is a required credential at the login endpoint. App role
pointed to by the `role_id` will have constraints set on it. This
...
#### Fetch the RoleID of the App role
...
<dl class="api">
  <dt>Description</dt>
  <dd>
  Reads the RoleID of an existing App role.
  </dd>
  ...
  <dd>
    ...
    <ul>
      <li>
        ...
        SecretID belonging to App role.
      </li>
    </ul>
  </dd>
